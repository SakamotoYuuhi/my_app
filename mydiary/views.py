from django.contrib.auth.mixins import LoginRequiredMixin

from django.urls import path, reverse_lazy
from django.views.generic import ListView, DetailView
from django.views.generic.edit import CreateView, UpdateView, DeleteView

from .models import MyDiary
from .forms import MyDiaryForm

class IndexView(ListView):
  queryset = MyDiary.objects.order_by('-id').all()
  template_name = 'mydiary/mydiary_index.html' # デフォルトは モデル名_list.html
  context_object_name = 'datas' # デフォルトはobject_list テンプレート側にデータを送る変数
  paginate_by = 10 # ページネーションする時の1ページあたりの数

# 詳細ページの処理
class DetailView(DetailView): 
  model = MyDiary
  template_name = 'mydiary/mydiary_detail.html'
  context_object_name = 'datas' # デフォルトはobject_list テンプレート側にデータを送る変数

# 投稿ページの処理
class CreateView(LoginRequiredMixin, CreateView):
  template_name = 'mydiary/mydiary_create.html'
  form_class = MyDiaryForm
  success_url = '/mydiary/'

  login_url = '/mydiary_login'

  # このメソッドは、有効なフォームデータがPOSTされたときに呼び出される
  def form_valid(self, form):
    path
    return super().form_valid(form)

# 編集ページの処理
class EditView(LoginRequiredMixin, UpdateView):
  model = MyDiary
  template_name = 'mydiary/mydiary_edit.html'
  form_class = MyDiaryForm
  context_object_name = 'datas' # デフォルトはobject_list テンプレート側にデータを送る変数

  login_url = '/mydiary_login'

  # このメソッドは、有効なフォームデータがPOSTされたときに呼び出される
  def form_valid(self, form):
    path
    return super().form_valid(form)

# 投稿されたデータを処理
class DeleteView(DeleteView):
  model = MyDiary
  context_object_name = 'datas' # デフォルトはobject_list テンプレート側にデータを送る変数
  success_url = reverse_lazy('mydiary_index')